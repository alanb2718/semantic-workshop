...
function mockResponse(url: string) {
  if (/badzone/.test(url)) {
    ...
    return [ { name: 'Big Zone', id: 5 }, { name: 'Northern Region', id: 8 }, { name: 'Southern Region', id: 9 } ];
  } else if (/GetFieldKeys/.test(url)) {
    return [ { key: 'service_body_bigint', description: 'Service Body ID'}, { key: 'start_time', description: 'Start Time'} ];
  } else {
